...
export default function NinjaGame() {
  ...
  async function buy() {
    setLoading(true);
    setDisableLoaderBtn(true);
    const { tournamentWriteContract } = await getTournamentContract(addr);
    let amountBN = ethers.utils.parseEther(amount.toString(), "18");
    let tx = await tournamentWriteContract.buyLife(amountBN);
    await tx.wait();
    dispatch(
        ...
        <div className="w-full rounded bg-[#0a1f2f] p-10">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <span className="font-bold text-2xl text-[#28dbd1]">0</span>
            </div>
            {!app.hasLife && (
