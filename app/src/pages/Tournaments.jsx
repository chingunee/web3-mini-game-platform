...
export default function Tournaments() {
  ...
  async function getAllTournamentAddresses() {
    try {
  ...
  async function getAllTournaments() {
    ...
    for (let id of allTournamentIds) {
      ...
      if (tournamentAddress != null) {
        ...
        let amountBN = await tournamentReadContract.prize();
        const tournamentPrize = ethers.utils.formatEther(amountBN);
        let tourEndTime = await tournamentReadContract.tournamentEndTime();
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 w-full h-64 mb-10">
              {tournaments.length > 0 &&
                tournaments.map((data, index) => {
                  ...
                  return <TournamentCard key={index} data={data} />;
                })}
            </div>
          )}
        </div>
